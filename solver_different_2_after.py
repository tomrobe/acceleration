...
def coupled_odes(x, y, phi, lamb, m, l, E):
    ...
    rho_double_prime = (
        ...
        + lamb * np.cos(phi + m*theta) * rho**l
    )
    theta_double_prime = (
        - 2 * rho_prime * theta_prime / rho
        + lamb * np.sin(phi + m*theta) * rho**(l-1)
    )
...
#    rho_prime_0 = np.sqrt(lamb/3) * rho_0**3
    ...
    term_potential_0 = lamb*np.cos(phi + m*theta_0) * rho_0**l
...
#    print(popt, pcov)
    ...
    kin_E = E**2 * rho
    interaction = lamb*np.cos(phi + m*theta) * rho**l
...
#    plt.ylim(-10,10)
    ...
    plt.xlabel('Clock')
    plt.ylabel('')
    plt.savefig('rhoprimeprimeoverrho2')
    plt.show()
...
##    plt.plot(time[blow:bup], theta_prime_smoothed_sg_sg[blow:bup], label=r'$\partial_{\chi} \theta$')
    ...
    plt.savefig('thetaprime2')
    plt.show()


    ################################    
    # Calculate epsilonh
    x = ((theta_prime)**2 + E**2) * (rho**2) + lamb*np.cos(phi + m*theta)*(rho**(l+1))
